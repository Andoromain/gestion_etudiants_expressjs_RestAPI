...
}).get("/api/classement/:niveau", (req, res, next) => {
    console.log("Classement des etudiants");
    mysql.conn.query("select etudiant.numEt, etudiant.nom , sum(note.note*matiere.coef)/sum(matiere.coef) as moyenne from etudiant,note,matiere where etudiant.numEt=note.numEt and matiere.codemat=note.codemat and etudiant.niveau=? group by etudiant.numEt  order by moyenne desc;", req.params.niveau, (err, rows, fields) => {
        if (err) throw err;
        res.status('200').json(rows);
    })
